...
class CheckList(object):
    ...
    def __call__(self, form, field):
        if field.data is not None:
            check_set = set(field.data.split(",").upper().trim())
        ...
        if (len(check_set - correct_set) > 0):
            raise ValidationError(
                'The following responses are not correct - {}'.format(check_set - correct_set))
    ...
    def __call__(self, form, field):
        ...
        if field.data is not None:
            ...
            if any(x for x in field.data if x not in valids):
                raise ValidationError(
                    "Incorrct: Your answer should only contain uppercase alphabetic characters or a gap symbol")
